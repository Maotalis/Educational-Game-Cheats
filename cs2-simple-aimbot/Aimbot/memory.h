...
T ReadMemory(uintptr_t addressToRead)
{
    ...
    ReadProcessMemory(Globals::hProcess, (PVOID)addressToRead, &rpmbuffer, sizeof(T), 0);
...
bool WriteMemory(uintptr_t address, const T& value) {
    SIZE_T bytesWritten;
    if (WriteProcessMemory(Globals::hProcess, (PVOID)address, &value, sizeof(T), &bytesWritten)) {
